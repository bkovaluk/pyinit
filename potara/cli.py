...
- --license: License type. Choices are 'MIT', 'Apache-2.0', 'GPL-3.0'.
  Default is 'MIT'.
...
def cli(
    ...
    description: str = typer.Option(
        "A new Python project.", help="Project description."
    ),
    author: str = typer.Option(
        "Your Name", help="Author name."
    ),
    email: str = typer.Option(
        "you@example.com", help="Author email."
    ),
    license: LicenseType = typer.Option(
        LicenseType.MIT, help="License type."
    ),
    venv: bool = typer.Option(
        False, help="Create a virtual environment."
    ),
    git: bool = typer.Option(
        False, help="Initialize a Git repository."
    ),
    ci: bool = typer.Option(
        False, help="Set up GitHub Actions workflow and Dependabot."
    ),
    ...
    # Create subdirectories before rendering templates
    subdirs = [
    ...
    if venv:
        ...
        subprocess.run(
            [os.sys.executable, "-m", "venv", venv_path],
            check=True,
        )
    ...
    if git:
        try:
            subprocess.run(
                ["git", "init"], cwd=project_dir, check=True
            )
            subprocess.run(
                ["git", "add", "."], cwd=project_dir, check=True
            )
            subprocess.run(
    ...
    typer.echo(
        f"\nProject '{project_name}' has been initialized!"
    )
